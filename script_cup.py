from coupe import make_cup
import tkinter as tk
import tkinter.filedialog

nom_saison = tk.filedialog.askdirectory(initialdir = '.', title = "Select data directory")+'/'
nom_res = tk.filedialog.askdirectory(initialdir = '.', title = "Select result directory")+'/'

criterias = ['diff', 'bp', 'conf']

additional_arguments = { 'prolongations' : True,
              'agg_dom' : 0,
              'agg_ext' : 0,
              'buts_ext_2' : True,
              'tab' : True,
              'terrain_neutre' : False,
              'bonus_dom' : 1.025,
              'pen_threshold' : 0.08,
              'proba_csc' : 0.003,
              'penalty_term' : 2.0,
              'proba_but_peno' : 0.7}

make_cup(nom_saison=nom_saison, resultat_saison=nom_res, coupe_nationale=False,
         aller_retour=True, finale_ar=False, 
         liste_chapeaux = [1,0], un_seul_tour=True,
         poules=False, nb_qualifies_poule=2, nb_equipe_poule=4,
         criterias=criterias, flag_exist=True, add_args=additional_arguments)